...
export enum TmaCartEntryActionTypes {
  ...
  CART_REMOVE_ENTRY_ANONYMOUS = '[Cart-entry] Tma Remove Entry Anonymous'
}
...
export class CartRemoveEntry extends StateUtils.EntityProcessesIncrementAction {
  readonly type = TmaCartEntryActionTypes.CART_REMOVE_ENTRY_ANONYMOUS;
  constructor(
    public payload: { cartId: string; userId: string; entryNumber: string, guid: string }
  ) {
    super(MULTI_CART_DATA, payload.cartId);
  }
}
